...
'''
注意注意！！！！！！！！！！
这个为老版的ui和main要想看看老版什么效果，讲这三个文件放置根目录即可运行。
'''
...
class CamConfig:
    ...
    def show_pic(self):
        # 全局变量
        # 在函数中引入定义的全局变量
        global EYE_AR_THRESH,EYE_AR_CONSEC_FRAMES,MAR_THRESH,MOUTH_AR_CONSEC_FRAMES,COUNTER,TOTAL,mCOUNTER,mTOTAL,ActionCOUNTER,Roll,Rolleye,Rollmouth,emotionCOUNTER,countEMOS
        ...
        if success:
            ...
            ret, frame, label_emo = myframe.frametest(frame)
            lab, eye, mouth = ret
            # ret和frame，为函数返回
            ...
            emotionCOUNTER += 1
            print(label_emo)
            countEMOS[label_emo] += 1
            if emotionCOUNTER == 16:
                print(EMOTIONS[countEMOS.index(max(countEMOS))])
                print('max:', countEMOS.index(max(countEMOS)))
                emotionCOUNTER = 0
                countEMOS = [0, 0, 0, 0]
